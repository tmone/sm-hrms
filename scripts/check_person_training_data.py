...
def check_person_training_data(person_id="PERSON-0020"):
    ...
    print(f"🔍 Checking training data for {person_id}\n")
    
    # 1. Check person folder
    person_dir = Path("processing/outputs/persons") / person_id
    if not person_dir.exists():
        print(f"❌ Person folder not found: {person_dir}")
        return
    
    # Count images
    person_images = list(person_dir.glob("*.jpg"))
    print(f"📁 Person folder: {person_dir}")
    ...
    if dataset_dir.exists():
        ...
        print(f"\n⚠️  No dataset folder found for {person_id}")
    ...
    if person_in_models:
        print(f"✅ Found in {len(person_in_models)} models:")
        ...
        print(f"❌ {person_id} NOT found in any trained models!")
    ...
    if not latest_dataset:
        print(f"❌ {person_id} NOT found in any training datasets!")
    ...
    if person_images:
        ...
        if sizes:
            ...
            if avg_width < 128:
                print(f"   ⚠️  Images might be too small for good recognition!")
    ...
    if not dataset_images:
        print(f"❌ No images in dataset folder - need to confirm images first")
        print(f"   → Go to person review page and confirm correct images")
    
    if not person_in_models:
        print(f"❌ Not in any trained models - need to retrain")
        print(f"   → Run: python scripts/retrain_person_model.py")
    
    if len(person_images) < 10:
        print(f"⚠️  Only {len(person_images)} images - might need more for better recognition")
