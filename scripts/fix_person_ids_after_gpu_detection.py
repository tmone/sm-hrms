...
print("[CONFIG] Fixing Person IDs with Recognition\n")
...
try:
    ...
    if recognizer.inference is None:
        print("[ERROR] Recognition model not loaded - cannot fix IDs")
        print("The model needs to be retrained or fixed")
        sys.exit(1)
    else:
        print("[OK] Recognition model loaded")
except Exception as e:
    print(f"[ERROR] Failed to load recognition: {e}")
    sys.exit(1)
...
for folder in recent_folders:
    ...
    if recognition_results:
        ...
        if avg_confidence > 0.7 and recognized_id != person_id:
            print(f"   [TARGET] Recognized as {recognized_id} (confidence: {avg_confidence:.2f})")
            ...
            if target_folder.exists():
                print(f"   [FOLDER] Merging into existing {recognized_id} folder")
                ...
                print(f"   [OK] Moved {moved_count} images")
                
                # Remove empty folder
                try:
                    shutil.rmtree(folder)
                    print(f"   [DELETE] Removed duplicate folder {person_id}")
                except:
                    pass
                    
            else:
                print(f"   [LOG] Renaming folder to {recognized_id}")
                try:
...
if merge_map:
    print(f"\n[PROCESSING] Merges performed:")
    for old_id, new_id in merge_map.items():
        print(f"   {old_id} -> {new_id}")

print("\n[OK] Person ID fixing complete!")
...
if merge_map:
    ...
    print(f"\n[LOG] Merge log saved to: {log_file}")